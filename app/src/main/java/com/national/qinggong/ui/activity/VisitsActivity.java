...
public class VisitsActivity extends BaseActivity {
    ...
    public void newsaleData() {
        newArrival_DataAdapter = new JoneBaseAdapter<VisitsBean.Data.Lists.Datas>(fans_recycleview, R.layout.item_my_fans) {

            @Override
            public void fillItemData(BGAViewHolderHelper helper, int position, VisitsBean.Data.Lists.Datas model) {
                ...
                Glide.with(VisitsActivity.this).load(model.user.avatarUrl).into((ImageView) helper.getView(R.id.iv_user_head));
            }
        };
        LinearLayoutManager layoutManager = new LinearLayoutManager(VisitsActivity.this, LinearLayoutManager.VERTICAL, false);
            ...
            public void onItemChildClick(ViewGroup parent, View childView, int position) {
                switch (childView.getId()) {
                    case R.id.tv_number:
                        ...
                        LiveAnchorDetailActivity.open(VisitsActivity.this, user_id+ "");
                        break;
                }}
        });
    ...
    private void getFans() {
        ...
        RetrofitClient.getApiService(API.APP_QING_GONG)
                .getVisits(map)
                .compose(RequestManager.<VisitsBean>applyIoSchedulers())
                ...
                .subscribe(new Consumer<VisitsBean>() {
                               @Override
                               public void accept(VisitsBean visitsBean) throws Exception {
                                   if (visitsBean.code == 1) {
                                       if (visitsBean.data.list.data!=null&visitsBean.data.list.data.size()>0){
                                           tv_number.setText(visitsBean.data.list.data.size()+"");
                                           newArrival_DataAdapter.setData(visitsBean.data.list.data);
